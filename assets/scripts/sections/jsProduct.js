Shopify.theme.jsProduct = {
  ...
  enableSlideshow(selector, settings) {
    ...
    let thumbnailsEnabled;
    let thumbnailsSliderEnabled;
    let thumbnailsPosition;
    let arrowsEnabled;
    let slideshowSpeed;
    let slideshowTransition;
    if (settings) {
    ...
    $productGallery.on('ready.flickity', function() {
      ...
      $slides.each(function(index, slide) {
        ...
        let mediaType = $(slide).data('media-type') || $(slide).find('[data-media-type]').data('media-type');
        let videoID;
    ...
    $productGallery.flickity({
      ...
      watchCSS: this.template === 'image-scroll' && !$productGallery.hasClass('js-gallery-modal') ? true : false, // Disables Flickity for main product gallery on image-scroll template
    ...
    if (($sliderArrows || $thumbnails) && isScreenSizeLarge()) {
      ...
      $thumbnails.keypress(function(event) {
        ...
        if(event.which == 13) {
          ...
          // Find out media type of featured media slide
          const mediaType = $selectedSlide.data('media-type') || $selected.find('[data-media-type]').data('media-type');
    ...
    if (thumbnailsEnabled == true && thumbnailsSliderEnabled == true && $slides.length > 1) {
      ...
      if(isScreenSizeLarge()) {
        ...
        if(thumbnailsPosition == 'right-thumbnails' || thumbnailsPosition == 'left-thumbnails') {
          ...
          setTimeout(function() {
            ...
            let $productGalleryHeight = $productGallery.height();
            $thumbnailProductGallery.css('max-height', $productGalleryHeight);

            // Determine the new height of the thumbnail product gallery
            $productGallery.on('change.flickity', function() {
              let $productGalleryHeight = $productGallery.height();
              $thumbnailProductGallery.css('max-height', $productGalleryHeight);
            });
      ...
      } else { //Otherwise create standard thumbnail slider
        $thumbnailProductGallery.flickity({
          cellAlign: 'center',
          contain: true,
          groupCells: '80%',
